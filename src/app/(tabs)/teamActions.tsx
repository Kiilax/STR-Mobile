...
import { useTeamActionsStore, LocalTeamAction } from "@/hooks/useTeamActionsStore";
...
export default function TeamActionsScreen() {
  ...
  const {
    ...
    onUrlFound: useCallback(async (foundUrl: string, foundEventId: number, foundTeamId: string | null) => {
      if (foundTeamId) {
        ...
        Alert.alert("Erreur", "Identifiant d'équipe manquant dans le QR Code.");
        handleRescanPress();
      }
    }, []),
  });
  ...
  }, [url, currentTeamId, hasScannedQR, fetchTeamActions, setTeamActionsFromApi]);
      ...
      >
        <View style={styles.actionContentSide}>
          ...
          <View style={styles.actionTexts}>
            <Text style={[styles.actionTitle, item.done && styles.strikethroughText]}>
              {isDropoff ? "Déposer équipement" : "Retirer équipement"}
            </Text>
            <Text style={[styles.actionSubtitle, item.done && styles.strikethroughText]}>
        ...
        <View style={styles.checkboxContainer}>
          <Ionicons 
            ...
            size={28} 
            color={item.done ? (colors.dark.tint || "#2196F3") : "rgba(255,255,255,0.3)"} 
          />
        </View>
      </TouchableOpacity>
    );
    ...
    <SafeAreaView style={styles.container}>
      {!hasScannedQR ? (
        <View style={styles.centerContent}>
          <Ionicons name="qr-code-outline" size={100} color={colors.dark.text} style={{ opacity: 0.5, marginBottom: 30 }} />
        ...
        <View style={{flex: 1}}>
            ...
            <View style={styles.listContainer}>
              {loadingActions ? (
                ...
                <FlatList
                  ...
                  ListEmptyComponent={
                    <Text style={[styles.infoText, {marginTop: 40}]}>Aucune action prévue.</Text>
                  }
                />
              )}
            </View>
          ...
          <View style={styles.footerContainer}>
              ...
              >
                  ...
                  <Text style={styles.mainButtonText}>Scanner un nouveau planning</Text>
              </TouchableOpacity>
          </View>
        </View>
      )}
