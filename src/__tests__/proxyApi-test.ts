...
describe("ProxyApi", () => {
  ...
  describe("request", () => {
    it("makes request with correct parameters", async () => {
      const mockResponse = {
        ...
        text: jest
          .fn()
          .mockResolvedValue(
            JSON.stringify({ success: true, data: { foo: "bar" } })
          ),
      };
    ...
    it("handles non-200 HTTP status", async () => {
      ...
      await expect(
        ProxyApi.request("https://site.com", "/404")
    ...
    it("handles API error response (success: false)", async () => {
      const mockResponse = {
        ...
        text: jest
          .fn()
          .mockResolvedValue(
            JSON.stringify({ success: false, error: "Custom Logic Error" })
          ),
      };
      mockFetch.mockResolvedValue(mockResponse);
      ...
      const consoleSpy = jest
        ...
        .mockImplementation(() => {});
      await expect(
        ProxyApi.request("https://site.com", "/err")
      ).rejects.toThrow("Custom Logic Error");
